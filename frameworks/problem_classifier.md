...
**When confidence < 0.7 after clarification attempts:**

1. **Maximum Clarification Attempts:** 2 attempts to improve confidence through clarifying questions

2. **After 2 Failed Attempts:**
   - Default to safest classification option
   - Category: CREATION (requires R-I-S-E, most thorough)
   - Domain: general_reasoning (universal, no domain assumptions)
   - Framework: R-I-S-E (handles complexity best)
   - Explicitly state low confidence and rationale

3. **User Override:**
   - Always allow user to explicitly specify classification
   - Accept manual category/domain/framework selection
   - Proceed with user-specified classification
   - Still document confidence if user selection seems problematic

4. **Learning:**
   - Log ambiguous classifications
   - Store user corrections for future improvement
   - Update classifier patterns based on corrections

### Persistent Low Confidence Protocol
...
low_confidence_handling:
  ...
  after_max_attempts:
    category_default: "CREATION"
    domain_default: "general_reasoning"
    framework_default: "R-I-S-E"
    confidence_statement: "Proceeding with default classification due to low confidence"
    
  user_override:
    allowed: true
    message: "You may override this classification by specifying: [DOMAIN: X] [FRAMEWORK: Y]"
...
**When no clear domain signals detected:**

1. **Ask User Explicitly**
   - List any weak signals detected (if any)
   - Present available domain options
   - Request user specification

2. **Default Behavior**
   - Use general_reasoning domain
   - Warn that domain-specific standards won't be applied
   - Suggest domain module creation if pattern emerges

3. **Conflicting Signals**
   - If multiple domains detected with similar confidence:
     - List detected domains
     - Ask user to specify primary domain
     - Use multi-domain orchestration if user confirms multiple domains

### Ambiguous Problem Statement

**When problem contains conflicting signals:**

1. **Decompose Problem**
   - Identify distinct sub-problems
   - Classify each separately
   - If multiple categories: Escalate to Layer 2 (Orchestration)

2. **Ask for Clarification**
   - Present identified sub-problems
   - Ask user to prioritize primary goal
   - Classify based on primary goal

3. **Default Strategy**
   - If truly ambiguous: Use CREATION + R-I-S-E (safest, most thorough)
   - Document all detected signals
   - Allow user to override with explicit classification
...
1. **Record Correction**
   ...
   - Problem signature (keywords, domain signals)
   - Confidence of original classification

2. **Update Classifier**
   - Adjust signal weights based on correction
   - Identify misclassification patterns
   - Improve future accuracy

3. **Learning Record Format**
   ```yaml
   classification_correction:
     ...
     corrected:
       category: "TRANSFORMATION"
     problem_signature: "refactor, migrate, convert"
     lesson: "Strong TRANSFORMATION signals should override weak CREATION signals"
   ```
...
LOW CONFIDENCE (< 0.7):
  → Ask clarifying question (max 2 attempts)
  → After 2 attempts: Default to CREATION + R-I-S-E
  → Allow user override

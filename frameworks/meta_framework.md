...
### Layer 1 → Layer 2 Escalation Decision Tree
...
Layer 1 Execution
    ...
    │       Indicators:
    │       - Multiple categories identified (e.g., CREATION + UNDERSTANDING)
    │       - User request explicitly mentions multiple deliverables
    │       - Problem decomposition reveals different framework needs
    │
    ...
    │       - Multiple domains involved (e.g., MHC + Web)
    ...
    │
    └─→ Dependencies between sub-problems identified?
        └─→ YES: Escalate to Layer 2
            Indicators:
            - Sequential execution required
            - Handoff artifacts needed
            - Sub-problems depend on each other
            - Coordination needed
```

### Layer 2 → Layer 3 Escalation Decision Tree
...
Layer 2 Orchestration
    ...
    │       Conditions:
    │       - Sub-problem doesn't match any framework purpose
    ...
    │   └─→ YES: Escalate to Layer 3
    │       Conditions:
    │       - Similar failures with existing frameworks
    │       - Systematic gap identified
    │       - Pattern indicates framework limitation
    ...
    │       - User requests framework creation
    │
    └─→ Framework gap pattern identified?
        └─→ YES: Escalate to Layer 3
            - Multiple similar problems need new approach
            - Existing frameworks fundamentally insufficient
            - Need GENESIS layer to create new framework
```

### Explicit Trigger Table

| From | To | Trigger | Decision Criteria |
|------|-----|---------|------------------|
| Layer 1 → Layer 2 | Problem requires multiple frameworks | Multiple categories or explicit multi-deliverable request |
| Layer 1 → Layer 2 | Cross-cutting concerns detected | Multiple domains, systems, or artifact types involved |
| Layer 1 → Layer 2 | Single framework stuck after 3 iterations | No progress after 3 iterations, quality not improving |
| Layer 1 → Layer 2 | Dependencies between sub-problems | Sequential execution needed, handoff artifacts required |
| Layer 2 → Layer 3 | No framework covers a sub-problem | Sub-problem doesn't match any framework, novel problem type |
| Layer 2 → Layer 3 | Repeated failures indicate systemic gap | Pattern of similar failures, framework limitation identified |
| Layer 2 → Layer 3 | User explicitly requests new framework | User identifies gap, requests framework creation |

### See Also

- [Edge Case Handling Guide](../docs/edge_case_handling.md) - Detailed escalation decision trees
- [Error Handling Protocol](error_handling_protocol.md) - Error-based escalation triggers
